...
function JobAlertsPage() {
  ...
  const load = async () => {
    ...
    if (result.ok) { setAlerts(result.data.data.items); setLimits(result.data.data.limits); setError(''); }
    else setError(result.error.message || 'We could not load your job alerts.');
    setIsLoading(false);
  };
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (result.ok) { setForm(emptyForm); await load(); } else setError(result.error.message || 'We could not create this alert.');
    setIsSaving(false);
  };
  ...
  const toggle = async (alert: JobAlert) => {
    ...
    else setError(result.error.message || 'We could not update this alert.');
  };
  ...
  const remove = async (alertId: string) => {
    ...
    else setError(result.error.message || 'We could not delete this alert.');
  };
  ...
  return <main className="seeker-layout__main job-alerts-page">
    ...
    <section className="seeker-card job-alerts-page__panel"><div className="job-alerts-page__heading"><div><h2>Create an alert</h2><p>{limits?.alerts === null || limits?.alerts === undefined ? 'Your plan limit is loading.' : Number.isFinite(limits.alerts) ? `${alerts.length} of ${limits.alerts} alerts used` : 'Unlimited alerts on your plan.'}</p></div></div>
      ...
      {error ? <p className="job-alerts-page__error" role="alert">{error}</p> : null}
    </section>
